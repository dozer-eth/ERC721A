  ...
  function () {
    context(`${contract}`, function () {
      beforeEach(async function () {
        ...
        this.startClaimTokenId = this.erc721a.startClaimTokenId ? (await this.erc721a.startClaimTokenId()).toNumber() : 1;
        this.startMintTokenId = this.erc721a.startMintTokenId ? (await this.erc721a.startMintTokenId()).toNumber() : 10001;
      });
      ...
      context('with no minted or claimed tokens', async function () {
        ...
        it('has 0 totalClaimed', async function () {
          const totalClaimed = await this.erc721a.totalClaimed();
      ...
      context('with minted tokens', async function () {
        ...
        describe('ERC721Metadata support', async function () {
          ...
          describe('tokenURI', async function () {
            it('sends an emtpy uri by default', async function () {
              const uri = await this.erc721a['tokenURI(uint256)'](10002);
              expect(uri).to.eq('');
            })

            it('reverts when tokenid is invalid', async function () {
              await expect(this.erc721a['tokenURI(uint256)'](10042)).to.be.reverted;
            })
          })
        })
        ...
        describe('exists', async function () {
          it('verifies valid tokens', async function () {
            for (let tokenId = this.startMintTokenId; tokenId < 6 + this.startMintTokenId; tokenId++) {
          ...
          it('verifies invalid tokens', async function () {
            expect(await this.erc721a.exists(6 + this.startMintTokenId)).to.be.false;
          });
        });
        ...
        describe('ownerOf', async function () {
          it('returns the right owner', async function () {
            expect(await this.erc721a.ownerOf(0 + this.startMintTokenId)).to.equal(this.addr1.address);
            expect(await this.erc721a.ownerOf(1 + this.startMintTokenId)).to.equal(this.addr2.address);
            expect(await this.erc721a.ownerOf(5 + this.startMintTokenId)).to.equal(this.addr3.address);
          });

          it('reverts for an invalid token', async function () {
            await expect(this.erc721a.ownerOf(10010)).to.be.revertedWith('OwnerQueryForNonexistentToken');
          });
        });

        describe('approve', async function () {
          beforeEach(function () {
            this.tokenId = this.startMintTokenId;
            this.tokenId2 = this.startMintTokenId + 1;
          });
          ...
          it('does not get approved for invalid tokens', async function () {
            await expect(this.erc721a.getApproved(10010)).to.be.revertedWith('ApprovalQueryForNonexistentToken');
          });
        });
        ...
        context('test transfer functionality', function () {
          const testSuccessfulTransfer = function (transferFn) {
            beforeEach(async function () {
              this.tokenId = this.startMintTokenId + 1;
            ...
            describe('safeTransferFrom', function () {
              ...
              it('validates ERC721Received', async function () {
                await expect(this.transferTx)
                  ...
                  .withArgs(this.addr2.address, this.addr2.address, 1 + this.startMintTokenId, '0x', GAS_MAGIC_VALUE);
              });
            });
          });
        ...
        describe('_burn', async function() {
          beforeEach(function () {
            this.tokenIdToBurn = this.startMintTokenId;
          });
      ...
      context('with claimed tokens', async function () {
        beforeEach(async function () {
          const [owner, addr1, addr2, addr3] = await ethers.getSigners();
          this.owner = owner;
          this.addr1 = addr1;
          this.addr2 = addr2;
          this.addr3 = addr3;
          await this.erc721a['claim(address,uint256,uint256)'](addr1.address, this.startClaimTokenId+1,1);
          await this.erc721a['claim(address,uint256,uint256)'](addr2.address, this.startClaimTokenId+2,2);
          await this.erc721a['claim(address,uint256,uint256)'](addr3.address, this.startClaimTokenId+5,3);
        });
        ...
        describe('ERC721Metadata support', async function () {
          ...
          describe('tokenURI', async function () {
            it('sends an emtpy uri by default', async function () {
              const uri = await this.erc721a['tokenURI(uint256)'](this.startClaimTokenId+2);
              expect(uri).to.eq('');
            })
        ...
        describe('exists', async function () {
          it('verifies valid tokens', async function () {
            const validTokenIndexes = [1,2,3,5,6,7]
            for (let tokenIdIndex = 0; tokenIdIndex < validTokenIndexes.length; tokenIdIndex++) {
              const tokenId = this.startClaimTokenId+validTokenIndexes[tokenIdIndex]
              const exists = await this.erc721a.exists(tokenId);
              expect(exists).to.be.true
            }
          });
          ...
          it('verifies invalid tokens', async function () {
            const invalidTokenIndexes = [0,4,8]
            for (let tokenIdIndex = 0; tokenIdIndex < invalidTokenIndexes.length; tokenIdIndex++) {
              const tokenId = this.startClaimTokenId+invalidTokenIndexes[tokenIdIndex]
              const exists = await this.erc721a.exists(tokenId);
        ...
        context('_totalClaimed', async function () {
          it('has 6 totalClaimed', async function () {
            const totalClaimed = await this.erc721a.totalClaimed();
            expect(totalClaimed).to.equal('6');
          });
        });
        ...
        describe('ownerOf', async function () {
          it('returns the right owner', async function () {
            expect(await this.erc721a.ownerOf(1 + this.startClaimTokenId)).to.equal(this.addr1.address);
            expect(await this.erc721a.ownerOf(2 + this.startClaimTokenId)).to.equal(this.addr2.address);
            expect(await this.erc721a.ownerOf(3 + this.startClaimTokenId)).to.equal(this.addr2.address);
            expect(await this.erc721a.ownerOf(5 + this.startClaimTokenId)).to.equal(this.addr3.address);
            expect(await this.erc721a.ownerOf(6 + this.startClaimTokenId)).to.equal(this.addr3.address);
            expect(await this.erc721a.ownerOf(7 + this.startClaimTokenId)).to.equal(this.addr3.address);
          });

          it('reverts for an invalid token', async function () {
            await expect(this.erc721a.ownerOf(0 + this.startClaimTokenId)).to.be.revertedWith('OwnerQueryForNonexistentToken');
            await expect(this.erc721a.ownerOf(4 + this.startClaimTokenId)).to.be.revertedWith('OwnerQueryForNonexistentToken');
            await expect(this.erc721a.ownerOf(8 + this.startClaimTokenId)).to.be.revertedWith('OwnerQueryForNonexistentToken');
            await expect(this.erc721a.ownerOf(9 + this.startClaimTokenId)).to.be.revertedWith('OwnerQueryForNonexistentToken');
          });
        });

        describe('approve', async function () {
          beforeEach(function () {
            this.tokenId = this.startClaimTokenId + 1;
            this.tokenId2 = this.startClaimTokenId + 2;
          });
        ...
        context('test transfer functionality', function () {
          const testSuccessfulTransfer = function (transferFn) {
            beforeEach(async function () {
              this.tokenId = this.startClaimTokenId + 2;
          ...
          context('successful transfers', function () {
            ...
            describe('safeTransferFrom', function () {
              ...
              it('validates ERC721Received', async function () {
                await expect(this.transferTx)
                  .to.emit(this.receiver, 'Received')
                  .withArgs(this.addr2.address, this.addr2.address, 2 + this.startClaimTokenId, '0x', GAS_MAGIC_VALUE);
              });
            });
          });
        ...
        describe('_burn', async function() {
          beforeEach(function () {
            this.tokenIdToBurn = this.startClaimTokenId + 1;
          });
      ...
      context('mint', async function () {
        ...
        describe('safeMint', function () {
          it('successfully mints a single token', async function () {
            ...
            await expect(mintTx)
              .to.emit(this.erc721a, 'Transfer')
              .withArgs(ZERO_ADDRESS, this.receiver.address, this.startMintTokenId);
            await expect(mintTx)
              .to.emit(this.receiver, 'Received')
              .withArgs(this.owner.address, ZERO_ADDRESS, this.startMintTokenId, '0x', GAS_MAGIC_VALUE);
            expect(await this.erc721a.ownerOf(this.startMintTokenId)).to.equal(this.receiver.address);
          });
          ...
          it('successfully mints multiple tokens', async function () {
            ...
            for (let tokenId = this.startMintTokenId; tokenId < 5 + this.startMintTokenId; tokenId++) {
              await expect(mintTx)
                .to.emit(this.erc721a, 'Transfer')
        ...
        describe('mint', function () {
          ...
          it('does not revert for non-receivers', async function () {
            ...
            expect(await this.erc721a.ownerOf(this.startMintTokenId)).to.equal(nonReceiver.address);
          });
...
describe(
  'ERC721A override _startMintTokenId()',
  createTestSuite({ contract: 'ERC721AStartTokenIdMock', constructorArgs: ['Azuki', 'AZUKI', 2, 10002] })
);
